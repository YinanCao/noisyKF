function [out] = fit_noisyDelta_cfrule_Bavard_2alpha_2zeta(cfg)
...
pnam = cell(1,6); % name
pmin = nan(1,6);  % minimum value
pmax = nan(1,6);  % maximum value
pini = nan(1,6);  % initial value
pplb = nan(1,6);  % plausible lower bound
ppub = nan(1,6);  % plausible upper bound
pfun = cell(1,6); % prior function (empty = uniform)
...
if ~cfg.cfrule && isfield(cfg,'delta') && cfg.delta == 0
    pfun{3} = @(x)0; % added handling for delta = 0 while ~cfrule JL
else
    pfun{3} = @(x)betapdf(x,1,9);
end
% 4/ learning noise
pnam{4} = 'zeta_c';
switch nstype
...
switch nstype
    ...
    case 'white' % white noise ~ exp(0.1)
        ...
        pplb(5) = 0.01;
        ppub(5) = 0.5;
        pfun{5} = @(x)exppdf(x,0.1);
end
% 6/ policy temperature
pnam{6} = 'tau';
switch chrule
...
isnoisy = isempty(pfix{4}) || pfix{4} > 0;
...
out.cfg = cfg;
    ...
    function [pt,mt,vt,ut,st,wt] = getp(alpha_c,alpha_u,delta,zeta_c,zeta_u,tau)
        ...
        for itrl = 1:ntrl
            ...
            if strcmp(nstype,'weber')
                st(itrl,c,:) = zeta_c*abs(et(itrl,c,:));
            else
                st(itrl,c,:) = zeta_c;
            end
            mt(itrl,c,:) = mt(itrl,c,:)+alpha_c*et(itrl,c,:);
            for whichu = 1:length(uall)
                u = uall(whichu);
                if cfrule % counterfactual rule
                    et(itrl,u,:) = rt(itrl-1,u)-mt(itrl,u,:);
                    if strcmp(nstype,'weber')
                        st(itrl,u,:) = zeta_u*abs(et(itrl,u,:));
                    else
                        st(itrl,u,:) = zeta_u;
                    end
                    mt(itrl,u,:) = mt(itrl,u,:)+alpha_u*et(itrl,u,:);
                else % decay
                    st(itrl,u,:) = 0;
