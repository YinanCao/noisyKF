function [out] = fit_noisyKF_cfrule_Bavard_2zeta(cfg)
...
vs = 0.0016; % sampling variance, sigma = 4 (value 1 - 99)
v0 = vs;     % prior variance
...
out.cfg = cfg;
    ...
    function [pt,mt,vt,ut,st,wt] = getp(alpha,delta,zeta_c,zeta_u,tau)
        % get drift variance
        vd = fv(alpha)*vs;
        ...
        for itrl = 1:ntrl
            ...
            if strcmp(nstype,'weber')
                st(itrl,c,:) = zeta_c*kgain(c)*abs(et(itrl,c,:));
            else
                st(itrl,c,:) = zeta_c;
            end
            ...
            for whichu = 1:length(uall)
                ...
                if cfrule % counterfactual rule
                    ...
                    if strcmp(nstype,'weber')
                        st(itrl,u,:) = zeta_u*kgain(u)*abs(et(itrl,u,:));
                    else
                        st(itrl,u,:) = zeta_u;
                    end
            ...
            if norm_flag
                % range normalization:
                q = (q - prctile(q,0,2))./(prctile(q,100,2)-prctile(q,0,2));
            end
            % softmax:
