clear; clc; close all
maindir = '/mnt/homes/home030/yinancao/RLsampling2023/paperRepo/RLVOLUNP_ana/';
% maindir = '/Users/yinancaojake/Documents/Postdoc/UKE/project/projY/paper/RLVOLUNP_ana/';
cd(maindir)
% add directories
...
parfor s = 1:length(Data_stack)
    ...
    % fitfun       = @(x)fit_noisyDelta_cfrule_Bavard_1alpha_1zeta(x);
    % saveout{s}{1} = getvbmc(cfg,fitfun);
    % fitfun       = @(x)fit_noisyDelta_cfrule_Bavard_1alpha_2zeta(x);
    % saveout{s}{2} = getvbmc(cfg,fitfun);
    % fitfun       = @(x)fit_noisyDelta_cfrule_Bavard_2alpha_1zeta(x);
    % saveout{s}{3} = getvbmc(cfg,fitfun);
    fitfun       = @(x)fit_noisyDelta_cfrule_Bavard_2alpha_2zeta(x);
    saveout{s}{1} = getvbmc(cfg,fitfun);
    
end

time_str = strrep(mat2str(fix(clock)),' ','_');
save(['Bavard_Delta',norm_str,'_',time_str,'.mat'],'saveout','nfit_bads')
exit;
